...
def EPR(img):
    return np.power(img.flatten(), 2).mean()

def normalize(img):
    return cv2.normalize(np.abs(img).astype('float32'), None, 0.0, 1.0, cv2.NORM_MINMAX)
        
def compareRes(img,gen,multichannel=True,channel_axis=2):
    img=normalize(img)
    gen=normalize(gen)
    return {
        "mse":compare_mse(img,gen),
        "psnr":compare_psnr(img,gen,data_range=1), 
        "ssim":compare_ssim(img,gen,data_range=1,multichannel=multichannel,channel_axis=channel_axis),
        "pncc":pearsonr(img.flatten(), gen.flatten()),
        "emes":(EME(img),EME(gen)),
        "eprs":(EPR(img),EPR(gen))
    }  